      ...
      resource :documents do
        ...
        post :create do
          ...
          if document.persisted?
            ...
            error!({:error_code => ErrorList::NOT_CREATED, :error_message => document.errors.full_messages.to_s}, 422)
          end
        end
        ...
        delete :delete do
          ...
          return error!({:error_code => ErrorList::NOT_FOUND, :error_message => "Document not found"}, 404) unless document
          users=[]
          users << (document.project.try(:users) || []) << (document.task.try(:user) || [])
          return error!({:error_code => ErrorList::NOT_AUTHORIZED, :error_message => "Unauthorized document"}, 401) unless users.flatten.include? current_user
